<script lang="ts">
  ...
  function scanUpload(e) {
    ...
    reader.onload = function (event) {
      ...
      let msg_col = 1; // Default per the upload template

      const mc = data[0].indexOf("Message");
      msg_col = mc > -1 ? mc : data[0].indexOf("TextMessage");
      ...
      non_gsm = data.reduce((acc: [], row: []) => {
        let s = new Set(row[msg_col]);
        let diff = setDifference(s, gsm_set);
        const diff_str = Array.from(diff).join(" ");
        if (diff.size > 0) {
          const updated_row = [diff_str, ...row];
...
<div class="flex flex-col justify-center p-4 m-auto tau">
  ...
  {:else}
    <div class="flex flex-col justify-between h-full" in:fade>
      ...
      {#if non_gsm?.length > 0}
        <div>
          Possible issue detected in {non_gsm.length} of your templates.
        </div>
