<script lang="ts">
  ...
  function fileComplete(results, file) {
    ...
    // Tell the user if there are no issues found (nif)
    if (non_gsm.length === 0) nif = true;
  }
...
<main class="overflow-none">
  <div class="relative flex flex-col justify-center p-4 m-auto tau">
    {#if !scanning && non_gsm.length === 0}
      ...
      {#if nif}
        <div
          class="w-full flex items-center justify-center flex-col absolute top-36 left-0"
          in:fade
        >
          <div
            class="flex flex-col items-center justify-center bg-lime-500 w-32 h-32 rounded-full"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            ...
            <div class="font-semibold mt-2">No issues found!</div>
          </div>
        </div>
      {/if}
    {:else}
